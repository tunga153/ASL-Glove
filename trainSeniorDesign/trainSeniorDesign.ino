...
const float BEND_RESISTANCE = 55000.0; // resistance at 90 deg
...
{  
  ...
  Serial.print(String(","));
  Serial.print(String(getBinary(FLEX_PIN_RING_MIDDLE)));
  Serial.print(String(","));
  ...
  //printAngles(FLEX_PIN_RING_MIDDLE);
  Serial.println();
...
int standardizeOutput() {
  ...
  int signalToSend = getBinary(FLEX_PIN_THUMB) << 14 | getBinary(FLEX_PIN_POINTER_TOP) << 12
  | getBinary(FLEX_PIN_POINTER_MIDDLE) << 10 | getBinary(FLEX_PIN_MIDDLE_TOP) << 8
  | getBinary(FLEX_PIN_MIDDLE_MIDDLE) << 6 | getBinary(FLEX_PIN_RING_TOP) << 4
  | getBinary(FLEX_PIN_RING_MIDDLE) << 2 | getBinary(FLEX_PIN_PINKY_TOP);
